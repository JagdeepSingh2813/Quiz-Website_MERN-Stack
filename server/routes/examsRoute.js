...
router.post("/add-question-to-exam", authMiddleware, async (req, res) => {
  try {
    // 🛠️ Convert correctOption to its key (A, B, C, D) if it’s the value
    const correctKey = Object.keys(req.body.options).find(
      (key) => req.body.options[key] === req.body.correctOption
    );

    if (correctKey) {
      req.body.correctOption = correctKey;
    }
